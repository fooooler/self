...
#### **第十五、十六周：数据库存储、索引与事务**  
**学习内容**：  
理解数据库物理存储方式（数据按页存储，页构成文件），掌握行存储（元组属性连续存放）与列存储（单属性连续存放）的适用场景；学习B+树索引（支持高效范围查询）与哈希索引（适合等值查询）的原理；掌握查询处理步骤（解析→优化→执行）及事务ACID特性（原子性、一致性、持久性）。  

**自我评分**：8分  
**理由**：存储与索引的底层理论（如B+树结构、页管理机制）较为抽象，花费较多时间理解，尚未完全掌握实际应用中的优化技巧。
...
- **支撑依据**：第四周有网课截图记录，第九周有书籍案例笔记，第十三、十四周有ER图修改痕迹，能证明每周学习投入。  
